...
def status(use_fork_point_overrides, warn_on_yellow_edges):
    dfs_res = []
    ...
    def fp_sha(b):
        if b not in fp_sha_cached:
            try:
                fp_sha_cached[b], fp_branches_cached[b] = fork_point_and_containing_branch_defs(b, use_fork_point_overrides)
            except MacheteException:
...
def usage(c=None):
    ...
    long_docs = {
        ...
        "format": """
            ...
            The annotation doesn't influence the way 'git machete' operates other than that those annotations are displayed in the output of the 'status' subcommand.
